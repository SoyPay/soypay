...
bool CWalletDB::WriteAccountTx(uint256 hash, const CAccountTx& atx)
{
	nWalletDBUpdated++;
	return Write(make_pair(string("acctx"), hash), atx);
}

bool CWalletDB::EraseAccountTx(uint256 hash)
{
	nWalletDBUpdated++;
	return Erase(make_pair(string("acctx"), hash));
}

bool CWalletDB::WriteRegID(CKeyID keyID,const CRegID &regID)
{
	nWalletDBUpdated++;
	return Write(make_pair(string("regid"), keyID), regID);
}

bool CWalletDB::EraseRegID(CKeyID keyID)
{
	nWalletDBUpdated++;
	return Erase(make_pair(string("regid"), keyID));
}


bool CWalletDB::WriteScriptRegID(uint256 hash,const CRegID &regID)
{
	nWalletDBUpdated++;
	return Write(make_pair(string("sregid"), hash), regID);
}

bool CWalletDB::EraseScriptRegID(uint256 hash)
{
	nWalletDBUpdated++;
	return Erase(make_pair(string("sregid"), hash));
}
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if(strType == "regid")
        {
        	...
        	ssKey>>keyid;
        	ssValue>>regid;
        	pwallet->mapKeyRegID[keyid] = regid;
        }
        else if(strType == "sregid")
        {
           uint256 hash;
           CRegID regid;
           ssKey>>hash;
           ssValue>>regid;
           pwallet->mapScriptRegID[hash] = regid;
        }
        else if (strType == "acctx")
        {
            uint256 hash;
            ssKey >> hash;
            CAccountTx atx;
...
//                wss.fAnyUnordered = true;
            ...
            pwallet->AddToWallet(atx);
            //// debug print
            //LogPrint("INFO","LoadWallet  %s\n", wtx.GetHash().ToString());
            //LogPrint("INFO"," %12d  %s  %s  %s\n",
            //    wtx.vout[0].nValue,
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    LogPrint("INFO","Keys: %u plaintext, %u encrypted, %u w/ metadata, %u total\n",
...
DBErrors CWalletDB::FindWalletTx(CWallet* pwallet, vector<uint256>& vBlockHash)
{
    pwallet->vchDefaultKey = CPubKey();
    CWalletScanState wss;
    bool fNoncriticalErrors = false;
...
DBErrors CWalletDB::ZapWalletTx(CWallet* pwallet)
{
    ...
    DBErrors err = FindWalletTx(pwallet, vBlockHash);
    if (err != DB_LOAD_OK)
        return err;

    // erase each wallet TX
    for (auto& hash : vBlockHash) {
        if (!EraseAccountTx(hash))
            return DB_CORRUPT;
    }
    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(wallet.strWalletFile) || bitdb.mapFileUseCount[wallet.strWalletFile] == 0)
            {
                ...
                try {
                	...
                	ifstream source(pathSrc.string().c_str(), ios::binary);
                	ofstream dest(strDest.c_str(), ios::binary);
...
//#endif
                    ...
                    LogPrint("INFO","error copying wallet.dat to %s - %s\n", pathDest.string(), e.what());
                    return false;
                }
            }
        }
...
{
    nWalletDBUpdated++;
    return Write(boost::make_tuple(string("destdata"), address, key), value);
}
