...
bool static TestEncode(uint64_t in) {
    return false;//return in == CTxOutCompressor::DecompressAmount(CTxOutCompressor::CompressAmount(in));
}

bool static TestDecode(uint64_t in) {
    return false;//in == CTxOutCompressor::CompressAmount(CTxOutCompressor::DecompressAmount(in));
}
