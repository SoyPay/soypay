...
//        LogPrint("INFO","ERROR: SignatureHash() : nIn=%d out of range\n", nIn);
//        return 1;
//    }
...
//            LogPrint("INFO","ERROR: SignatureHash() : nOut=%d out of range\n", nIn);
//            return 1;
//        }
//    }
...
private:
	...
	typedef boost::tuple<uint256, vector<unsigned char>, CPubKey> sigdata_type;
	set<sigdata_type> setValid;
	boost::shared_mutex cs_sigcache;

public:
	bool Get(const uint256 &hash, const vector<unsigned char>& vchSig, const CPubKey& pubKey) {
		boost::shared_lock<boost::shared_mutex> lock(cs_sigcache);

		sigdata_type k(hash, vchSig, pubKey);
		set<sigdata_type>::iterator mi = setValid.find(k);
		if (mi != setValid.end())
			return true;
		return false;
	}
	...
	void Set(const uint256 &hash, const vector<unsigned char>& vchSig, const CPubKey& pubKey) {
		...
		while (static_cast<int64_t>(setValid.size()) > nMaxCacheSize) {
			...
			set<sigdata_type>::iterator it = setValid.lower_bound(sigdata_type(randomHash, unused, unused));
			if (it == setValid.end())
				it = setValid.begin();
			setValid.erase(*it);
		}
...
public:
	...
	bool operator()(const CAccountID &accID) const {
		CKeyID keyID;
		return keystore->HaveKey(keyID);
	}
};
...
public:
	CAffectedKeysVisitor(const CKeyStore &keystoreIn, vector<CKeyID> &vKeysIn) :
	...
	void operator()(const CAccountID &accId) {
		CKeyID keyId;
		if (keystore.HaveKey(keyId))
			vKeys.push_back(keyId);
	}
...
bool CScriptCompressor::Compress(vector<unsigned char> &out) const {
	CKeyID keyID;
	if (IsToKeyID(keyID)) {
		out.resize(21);
		out[0] = 0x00;
...
bool CScriptCompressor::Decompress(unsigned int nSize, const vector<unsigned char> &in) {
	switch (nSize) {
	case 0x00:
		script.resize(25);
		script[0] = OP_DUP;
