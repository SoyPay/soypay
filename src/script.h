 ...
 */
typedef boost::variant<CNoDestination, CKeyID, CScriptID,CAccountID> CTxDestination;
...
public:
    ...
    void SetMultisig(int nRequired, const vector<CPubKey>& keys);


    void PrintHex() const
    {
        LogPrint("INFO","CScript(%s)\n", HexStr(begin(), end(), true).c_str());
    }
...
protected:
    ...
    bool Compress(vector<unsigned char> &out) const;
    unsigned int GetSpecialSize(unsigned int nSize) const;
    bool Decompress(unsigned int nSize, const vector<unsigned char> &out);
public:
    CScriptCompressor(CScript &scriptIn) : script(scriptIn) { }

    unsigned int GetSerializeSize(int nType, int nVersion) const {
        vector<unsigned char> compr;
        if (Compress(compr))
            return compr.size();
    ...
    void Unserialize(Stream &s, int nType, int nVersion) {
        ...
        if (nSize < nSpecialScripts) {
            vector<unsigned char> vch(GetSpecialSize(nSize), 0x00);
...
bool IsCanonicalPubKey(const vector<unsigned char> &vchPubKey, unsigned int flags);
bool IsCanonicalSignature(const vector<unsigned char> &vchSig, unsigned int flags);
